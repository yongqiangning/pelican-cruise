...
**声音分三路各管各的**：背景音乐、环境音（海浪 + 风声）、音效（海鸥 / 鹈鹕叫 / 车铃）互不牵连——关掉背景音乐，海浪照旧、海鸥照叫。背景音乐除了内置的实时合成氛围乐，还能上传自己的 MP3 当播放列表（可多选、循环播放；插件里会上传到本地保存，下次开新标签页还在）。插件里背景音乐**出厂是关的**，先听环境音。
...
| 背景音乐独立成一路（插件内） | 控制条第二行新增「**音乐**」按钮 + 浮窗：开关、音量、内置氛围乐 / 我的音乐 二选一、上传与删除本地音乐。它只管音乐总线，与「海浪」滑块（envBus：风声 + 海浪）和「声音」总开关（sfx：海鸥、鹈鹕叫、车铃）**互不影响**——关掉背景音乐，海浪照旧、海鸥照叫。插件里背景音乐**出厂是关的**（新标签页一开就放音乐太打扰）；主音量默认关着时，一动音乐音量会顺手把「声音」打开，免得「拖了没反应」 | `main.js` `index.template.html` `EXTENSION.md` |
| 上传的背景音乐跨标签页留存 | 新标签页每开一次都是全新加载，只用 ObjectURL 的话一关标签页曲子就没了、每次都得重传。现在上传的音频存进 **IndexedDB**（结构化克隆 Blob，不做 base64 膨胀；不用 `chrome.storage` 是因为它按 JSON 序列化存不了 Blob，配额也只有 10MB），音频引擎启动时装回播放列表；开关、音量、「用自己的歌还是内置」记在 localStorage（`pelican.music.*`）。**依旧零权限**——IndexedDB 不需要授权 | `src/music-store.js` `src/audio.js` `main.js` `extension/manifest.json` |
| 控制条两个浮窗按钮只留图标 | 「**快捷键**」与「**音乐**」原本都是「图标 + 文字」，两个按钮吃掉一整行的力气又重复（图标已经说明了一切）。现只留图标，并借机把按钮做成方块（32×32，窄窗口降到 30 / 28）——文字按钮靠 `padding: 7px 12px` 撑高度，只留图标后若沿用同一套内边距，图标会左右不居中且按钮被压扁；改成 `padding: 0` + flex 居中、尺寸按同行按钮高度取整，实测与文字按钮等高（差 1px）。按钮文字删掉后 `aria-label` 补上，`title` 保留，鼠标悬浮仍有说明。控制条因此变窄：1280 下 882→848px、820 下 785→750px、680 下 647→611px | `index.template.html` `EXTENSION.md` |
...
- **声音**：默认关，点一下开（海浪风声 + 音效；背景音乐另算），切走标签页自动静音
- **海浪**：「声音」右边的滑块单独调环境音（海浪 + 风声）音量，拖到 0 就是静音
- **音乐**：第二行最右边的 🎵 图标（只图标、无文字，悬浮有说明），点开是一个小面板——背景音乐开关、音量、内置氛围乐 / 我的音乐二选一、**上传本地音乐**（MP3 / M4A / WAV / OGG / FLAC，可多选循环播放）与删除。背景音乐与环境音是两路，关掉音乐不影响海浪、海鸥和车铃；插件里出厂是关的，上传的曲子存在本机、下次开新标签页还在
- **快捷键**：第一行最右边的 <kbd>⌨</kbd> 图标（只图标、无文字），鼠标悬浮浮出全部快捷键一览（<kbd>U</kbd> 显示 / 隐藏菜单排第一条），点一下可以钉住不消失
...
> 完整的**参数面板**（巡航速度、自动变速、日期时间、画质、各项音量）在**游戏页**里——点右上角工具条上的 <kbd>⚙️</kbd> 打开。壁纸版与新标签页把它刻意隐藏了（桌面上那个窗口连鼠标都不属于它），所以插件里调声音就用控制条上的「海浪」滑块和那个 🎵 图标的浮窗。

插件本体是 `dist/extension/`，Manifest V3，用 `chrome_url_overrides.newtab` 接管新标签页；**安装时不申请任何权限**（连 `storage` 都没要，设置与快捷网址存在扩展自己的 localStorage，你上传的背景音乐存 IndexedDB，两者都不需要授权），约 883 KB（含 4 个尺寸图标），完全离线——唯一会联网的是你点开的网页，以及为快捷网址取图标：浏览器按 `<img>` 去目标站点自己试几组常见图标路径（不经过第三方图标服务），或在你点「查找高清图标」并授权后读一次该站首页找它声明的图标（`optional_host_permissions`，装了不会弹警告，点了才申请，随时可取消）。`dist/pelican-newtab-extension.zip` 是它的压缩包，方便拷到别的机器再解压安装。你上传的音乐只存在本机、不会上传到任何地方。
