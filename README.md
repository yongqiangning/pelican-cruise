...
- **音乐**：第二行最右边的「**音乐**」按钮，点开是一个小面板——背景音乐开关、音量、内置氛围乐 / 我的音乐二选一、**上传本地音乐**（MP3 / M4A / WAV / OGG / FLAC，可多选循环播放）与删除。背景音乐与环境音是两路，关掉音乐不影响海浪、海鸥和车铃；插件里出厂是关的，上传的曲子存在本机、下次开新标签页还在
- **快捷键**：第一行最右边的 <kbd>⌨</kbd> 按钮，鼠标悬浮浮出全部快捷键一览（<kbd>U</kbd> 显示 / 隐藏菜单排第一条），点一下可以钉住不消失
...
> 完整的**参数面板**（巡航速度、自动变速、日期时间、画质、各项音量）在**游戏页**里——点右上角工具条上的 <kbd>⚙️</kbd> 打开。壁纸版与新标签页把它刻意隐藏了（桌面上那个窗口连鼠标都不属于它），所以插件里调声音就用控制条上的「海浪」滑块和「音乐」浮窗。
